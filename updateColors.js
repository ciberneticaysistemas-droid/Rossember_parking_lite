...
const replaceColorsInFile = (filePath) => {
  ...
  // Reemplazar colores principales por naranja y variaciones cálidas
  ...
  content = content.replace(/fuchsia-600/g, 'orange-600');
  content = content.replace(/violet-600/g, 'amber-600');
  
  // Reemplazar fondos oscuros estresantes por fondos limpios tipo amber/orange darks
  // O un tema más claro para que no sea hostigante.
  // "naranja estetico y no hostigante con la vista" significa probablemente crema claro, blancos, naranjas y grises.
  ...
  // Arreglar casos específicos de text-white que sí deben ser blancos (botones con fondo naranja ej bg-orange-500 text-gray-800 -> bg-orange-500 text-white)
  // En vez de usar regex complejo, dejemos que Tailwind haga el texto gris como base y los botones naranjas sigan teniendo texto oscuro o los podemos forzar.
  // Ya que es un reemplazo masivo, lo haremos simple:
